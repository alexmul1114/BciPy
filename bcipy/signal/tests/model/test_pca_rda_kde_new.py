...
class TestPcaRdaKdeModelInternals(ModelSetup):
    ...
    def test_kde_plot(self):
        ...
        for kernel in ["gaussian", "tophat", "epanechnikov"]:
            kde = KDE(kernel=kernel, scores=x, num_channels=x.shape[0], num_cls=1)
            kde.fit(x, y)
    ...
    def test_kde_values(self):
        pca = ChannelwisePCA(num_channel=self.num_channel, var_tol=0.5)
        rda = RDA()
        kde = KDE()

        pipeline = make_pipeline(pca, rda, kde)
    ...
    def test_rda(self):
        ...
        pipeline = make_pipeline(pca, rda)
